...
x_train, x_test, y_train, y_test = train_test_split(X, y_use, test_size=Test_size_, random_state=0, shuffle=True)
...
# 0.00631   0.00108
lasso_2 = linear_model.Lasso(alpha=0.0004, fit_intercept=False, positive=False,
                         random_state=0, max_iter=500000, selection='random', tol=1e-20)
ridge = linear_model.Ridge(alpha=4.7e-3, fit_intercept=False)
ann = MLPRegressor(hidden_layer_sizes=(10, 20, 20, 10), activation='relu', solver='adam', max_iter=10000, tol=1e-20, learning_rate_init=1e-4, n_iter_no_change=200)
...
weight = np.array(pd.read_excel('./w_0.5.xlsx')).reshape(-1)
...
# 将预测结果转换为一个DataFrame
predictions_matrix = np.array(predictions).T  # 转置后变成每个样本一个列，50个模型每行一个
predictions_df = pd.DataFrame(predictions_matrix)

# diagonal_array = np.array([np.diagonal(predictions_matrix[:, :, i]) for i in range(predictions_matrix.shape[2])]).T
# predictions_df = pd.DataFrame(diagonal_array)


# # 保存到Excel
# predictions_df.to_excel('predictions_output.xlsx', index=False)
#
# print("预测结果已保存到 'predictions_output.xlsx'.")
